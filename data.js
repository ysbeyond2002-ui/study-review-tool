...
function getDefaultData() {
  // 尝试从 localStorage 读取；如果数据为空，返回空数据对象
  try {
  ...
  return { math: [], chinese: [], english: [], words: [], stars: 0, last_login: '' };
}

// 从 seed.json（网站资源）导入备份数据
async function importSeedData() {
  try {
    ...
    if (raw) {
      const data = JSON.parse(raw);
      const total = (data.math?.length||0)+(data.chinese?.length||0)+(data.english?.length||0)+(data.words?.length||0);
      if (total > 0) return; // 已有数据，不覆盖
    }
    const resp = await fetch('./seed.json');
    if (!resp.ok) throw new Error('seed.json 加载失败');
    const seedData = await resp.json();
    if (seedData && seedData.math) {
      localStorage.setItem(STORAGE_KEY, JSON.stringify(seedData));
      console.log('✅ 已从 seed.json 导入备份数据');
    }
  } catch(e) {
    console.warn('导入备份数据失败（无备份文件或网络问题）', e);
  }
}
